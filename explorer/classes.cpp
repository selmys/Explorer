...
cavern::cavern(){
    description[0]='\0'; directions_out[0]='\0'; 
    explorers=0; stuff=0;
}

cavern::cavern(char *d, char *p, int l[], int n, int s){
...
explorer::explorer(){
    number=0; score=0; stuff=0; current_cavern=0;
}

explorer::explorer(int n, int c){
    number=n; current_cavern=c; score=0; stuff=0;
}

explorer::~explorer(){
    printf("deleting explorer number %d\n",number);
}

cave::cave(){ 
    number_of_caverns=number_of_explorers=0;
}

void cave::add(cavern *c){ 
    array_of_caverns[number_of_caverns++]=c;
}

void cave::add(explorer *e){
    array_of_explorers[number_of_explorers++]=e;
    array_of_caverns[e->current_cavern]->explorers |= (1 << e->number);
}

int cave::value(explorer *e){
...
void cave::put(char *ch, explorer *e){
    int i=1000,j=0;
    char c[1024]="",cc[1024]="";
    for(j=2;j<(int)strlen(ch);j++)
        c[j-2]=ch[j];
    c[j-2]='\0';
    for(j=0;j<16;j++)
...
void cave::get(char *ch, explorer *e){
    int i=1000,j=0;
    char c[1024]="",cc[1024]="";
    for(j=2;j<(int)strlen(ch);j++)
        c[j-2]=ch[j];
...
void cave::talk(char *ch, explorer *e){
    int j;
    char c[1024]="";
    printf("$%s$%ld\n",ch,strlen(ch));
    for(j=2;j<(int)strlen(ch);j++)
...
void cave::quit(explorer *e){
    value(e);
    write(e->number,"Q\n",2);
}

void cave::move(char d, explorer *e){
    int s,pos=0,i;
